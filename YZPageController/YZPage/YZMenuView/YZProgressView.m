...
{
    _gap = fabs(self.progress - posit);
    _sign = self.progress > posit ? -1 : 1;
...
- (void)drawRect:(CGRect)rect
{
    ...
    if (self.naughty) {
        ...
        if (rate <= 0.5) {
            ...
            CGFloat currentMaxX = currentX + currentMidX;
            endX = currentMidX + (nextMidX - currentMaxX) * rate * 2.0;
        }else{
    ...
    if (self.isTriangle) {
        ...
        CGContextAddLineToPoint(ctx, startX + width, 0);
